...
fn part_1(input: &str) -> Result<String, String> {
    ...
    for l in input.lines() {
        ...
        let winning_set = winning.split(' ').filter(|x| !x.is_empty()).collect::<HashSet<_>>();
        let played_set = played.split(' ').filter(|x| !x.is_empty()).collect::<HashSet<_>>();
        let mut count: u32 = 0;
...
fn part_2(input: String) -> Result<String, String> {
    let mut copies = vec![1_u32; input.lines().count()];
    for (i, l) in input.lines().enumerate() {
        // copies[i] += 1; // add 1 for the original (initialized at 0)
        let mut l_split = l.split(':');
        l_split.next(); // skip line header
        let lottery_nums = l_split.next().unwrap().trim();
        let mut split = lottery_nums.split('|');
        ...
        for j in 1..=count {
            copies[i+j as usize] += copies[i]; // 1 * copies[i]
        }
        // println!("copies at line {i} {copies:?}");
    }
